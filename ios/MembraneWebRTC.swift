...
class MembraneWebRTC: MembraneRTCDelegate {
    ...
    private func ensureCreated() throws{
        if(membraneRTC == nil) {
            throw Exception(name: "E_NO_MEMBRANERTC", description: "Client not created yet. Make sure to call create() first!")
        }
    }
    ...
    private func ensureEndpoints() throws {
        guard let _ = localEndpointId, !MembraneRoom.sharedInstance.endpoints.isEmpty else {
            throw Exception(name:"E_NO_ENDPOINTS", description: "No endpoints available. Ensure the connection is established or endpoints are present.")
        }
    }
    ...
    func connect(metadata: [String: Any], promise: Promise) {
        ...
        guard let localEndpointId = localEndpointId,
              var endpoint = MembraneRoom.sharedInstance.endpoints[localEndpointId] else {
            return
        }
    ...
    private func createCameraTrack(config: CameraConfig) throws -> LocalVideoTrack? {
        ...
        guard let simulcastConfig =  try getSimulcastConfigFromOptions(simulcastConfig: config.simulcastConfig) else { return nil }
        self.videoSimulcastConfig = simulcastConfig
        return membraneRTC?.createVideoTrack(
            videoParameters: videoParameters, metadata: config.videoTrackMetadata.toMetadata(), captureDeviceId: config.captureDeviceId
        )
    }
    private func setCameraTrackState(cameraTrack: LocalVideoTrack, isEnabled: Bool) throws {
        ...
        let isCameraEnabledMap = ["IsCameraOn": isEnabled]
        emitEvent(name: "IsCameraOn", data: isCameraEnabledMap)
    }
    private func addTrackToLocalEndpoint(_ track: LocalVideoTrack, _ metadata: Metadata) throws {
        ...
        if let localEndpointId = localEndpointId {
            ...
            MembraneRoom.sharedInstance.endpoints[localEndpointId]?.tracksMetadata[track.trackId()] = metadata
            emitEndpoints()
        }
    }
    ...
    private func removeTrackFromLocalEndpoint(_ track: LocalVideoTrack) throws {
        try ensureEndpoints()
        if let localEndpointId = localEndpointId {
            guard let localEndpoint = MembraneRoom.sharedInstance.endpoints[localEndpointId] else { return }
            MembraneRoom.sharedInstance.endpoints[localEndpointId] = localEndpoint.removeTrack(trackId: track.trackId())
            emitEndpoints()
        }
    }
    private func removeTrackFromLocalEndpoint(_ track: LocalAudioTrack){}
    func startMicrophone(config: MicrophoneConfig) throws -> Void {
        try ensureConnected()
        guard let microphoneTrack = membraneRTC?.createAudioTrack(metadata: config.audioTrackMetadata.toMetadata()) else { return }
    ...
    private func setMicrophoneTrackState(_ microphoneTrack: LocalAudioTrack, _ isEnabled: Bool) throws {
        ...
        let isMicEnabledMap = ["IsMicrophoneOn" : isEnabled]
        emitEvent(name: "IsMicrophoneOn", data: isMicEnabledMap)
    }
    ...
    private func setScreencastTrackState(isEnabled:Bool) throws {
        ...
        let isScreencastEnabled = ["IsScreencastOn": isEnabled]
        emitEvent(name: "IsScreencastOn", data: isScreencastEnabled)
    }
    func toggleScreencast(screencastOptions: ScreencastOptions) throws -> Void {
        try ensureConnected()
        guard let screencastExtensionBundleId = Bundle.main.infoDictionary?["ScreencastExtensionBundleId"] as? String else {
        ...
        guard let simulcastConfig  = try getSimulcastConfigFromOptions(simulcastConfig: screencastOptions.simulcastConfig) else {
            return
        }
        screencastSimulcastConfig = simulcastConfig
        screencastMaxBandwidth = getMaxBandwidthFromOptions(maxBandwidth: screencastOptions.maxBandwidth)
        ...
        localScreencastTrack = membraneRTC?.createScreencastTrack(
            ...
            onStart: { [weak self] screencastTrack in
                ...
                do {
                    ...
                    os_log("Error starting screencast: %{public}s", log: log, type: .error, String(describing: error))
                }
                ...
                do {
                    ...
                    os_log("Error stopping screencast: %{public}s", log: log, type: .error, String(describing: error))
                }
            })
        DispatchQueue.main.async {
            RPSystemBroadcastPickerView.show(for: screencastExtensionBundleId)
    ...
    func toggleScreencastTrackEncoding(encoding: String) throws -> [String: Any] {
        ...
        guard
            ...
            let simulcastConfig = toggleTrackEncoding(encoding: trackEncoding, trackId: trackId, simulcastConfig: screencastSimulcastConfig) else {
            throw Exception(name: "E_NOT_CONNECTED", description: "Client not connected to server yet. Make sure to call connect() first!")
        }
